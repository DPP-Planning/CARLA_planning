...
class car_mesh:
	...
	def contains_waypoint(self, waypoint_or_location, radius_m=0.0):
		...
		for i in range(len(vertices)):
			...
			def min_distance_reached(min_distance, waypoint_or_location):
				if isinstance(waypoint_or_location, carla.Waypoint):
					dist = self.center.location.distance(waypoint_or_location.transform.location)
				else:
					dist = self.center.location.distance(waypoint_or_location)
				return dist < min_distance

		return False or min_distance_reached(4, waypoint_or_location)
